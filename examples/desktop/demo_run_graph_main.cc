...
constexpr char kOutputHandedness[] = "output_handedness";
...
absl::Status RunMPPGraph() {
 ...
 ASSIGN_OR_RETURN(mediapipe::OutputStreamPoller poller_handedness,
                     graph.AddOutputStreamPoller(kOutputHandedness));
  //ASSIGN_OR_RETURN(mediapipe::OutputStreamPoller poller_presence,
  ...
  while (grab_frames) {
    ...
    if (is_landmark_present) {
       ...
       // if (poller_landmarks.Next(&packet_landmarks)) {
        if (!poller_landmarks.Next(&packet_landmarks) ||!poller_handedness.Next(&packet_handedness)) break;
              // do something
//            }
  //        }
   // auto &output_frame = packet.Get<mediapipe::ImageFrame>();
   ...
   // auto &output_detections = packet_detections.Get<std::vector<mediapipe::Detection>>();
    auto &output_handedness = packet_handedness.Get<std::vector<mediapipe::ClassificationList>>();
      //  LOG(INFO) << "handedness_size:"<< output_handedness.size()-output_landmarks.size();
       
   /* for (int i = 0; i < output_handedness.size(); i ++) {
                auto& h = output_handedness[i];
